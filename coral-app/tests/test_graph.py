...
def build(nodes, edges, port_table=PORT_TABLE):
    ...
    writes for a flat graph. Supplying it here keeps the node literals below about wiring and
    typing; that ``Graph`` *validates* the field (check 3) is pinned by ``TestQualifiedIds``, which
    bypasses this helper.

    Node ids are decimal integers throughout this file, because that is what ``Graph`` accepts: the
    ids therefore carry no meaning, and where a node's role matters to a case it is stated in the
    docstring.
    """
...
class TestOrdering:
    ...
    def test_simple_dag(self):
        ...
        graph = build(
            {
                "0": {"type": "float", "value": 1.0},
                "1": {"type": "float", "value": 2.0},
                "2": {"type": "add"},
            },
            {"0": edge("0", "2", 0), "1": edge("1", "2", 1)},
        )

        assert graph.order.index("0") < graph.order.index("2")
        assert graph.order.index("1") < graph.order.index("2")
    ...
    def test_parallel_edges_to_one_node(self):
        ...
        graph = build(
            {"0": {"type": "float", "value": 4.0}, "1": {"type": "add"}},
            {"0": edge("0", "1", 0), "1": edge("0", "1", 1)},
        )
    ...
    def test_diamond(self):
        ...
        graph = build(
            {
                "0": {"type": "float", "value": 9.0},  # the source
                "1": {"type": "sqrt"},  # the left branch
                "2": {"type": "sqrt"},  # the right branch
                "3": {"type": "add"},  # the sink they rejoin at
            },
            {
                "0": edge("0", "1", 0),
                "1": edge("0", "2", 0),
        ...
        assert sorted(graph.order) == ["0", "1", "2", "3"]
        assert graph.order.index("0") < graph.order.index("1") < graph.order.index("3")
        assert graph.order.index("0") < graph.order.index("2") < graph.order.index("3")
    ...
    def test_isolated_node(self):
        ...
        graph = build(
            {
                "0": {"type": "float", "value": 16.0},
                "1": {"type": "sqrt"},
                "2": {"type": "str", "value": "unconnected"},  # the isolated one
            },
            {"0": edge("0", "1", 0)},
        )

        assert sorted(graph.order) == ["0", "1", "2"]
        assert graph.order.index("0") < graph.order.index("1")
    ...
    def test_order_follows_the_graph_not_json_key_order(self):
        ...
        assert build(nodes, {}).order == ["0", "1", "2"]
    ...
    def test_ready_nodes_are_sorted_within_each_batch(self):
        ...
        nodes = {
            "3": {"type": "float", "value": 1.0},
            "0": {"type": "sqrt"},
            "1": {"type": "float", "value": 2.0},
            "2": {"type": "sqrt"},
        }
        edges = {"0": edge("3", "0", 0), "1": edge("1", "2", 0)}

        assert build(nodes, edges).order == ["1", "3", "0", "2"]
...
class TestEdgeEndpoints:
    ...
    def test_unknown_target_is_rejected(self):
        ...
        nodes = {"0": {"type": "float", "value": 1.0}}

        with pytest.raises(ValueError, match=r"Edge '0' names target node '99'"):
            build(nodes, {"0": edge("0", "99", 0)})
    ...
    def test_unknown_source_is_rejected(self):
        ...
        with pytest.raises(ValueError, match=r"Edge '0' names source node '99'"):
            build(nodes, {"0": edge("99", "0", 0)})
    ...
    def test_numeric_endpoint_ids_are_coerced_to_strings(self):
        ...
        edges = {"0": {"source": 1, "target": 2, "source_output": 0, "target_input": 0}}
    ...
    def test_edge_missing_a_required_key_is_rejected(self):
        ...
        nodes = {"0": {"type": "float", "value": 1.0}, "1": {"type": "sqrt"}}
        edges = {"0": {"source": "0", "target": "1", "source_output": 0}}

        with pytest.raises(ValueError, match=r"Edge '0' declares no 'target_input'"):
            build(nodes, edges)
...
class TestNodeTypes:
    ...
    def test_unknown_node_type_is_rejected(self):
        ...
        with pytest.raises(ValueError, match=r"Node '0' has unknown type 'nope'"):
            build({"0": {"type": "nope"}}, {})
...
class TestNodeIds:
    ...
    def test_integer_node_ids_are_coerced_to_strings(self):
        ...
        graph = build({0: {"type": "float", "value": 1.0}, 1: {"type": "show"}}, {"0": edge(0, 1)})
    ...
    @pytest.mark.parametrize("node_id", ["a", "", "-1", "01", "1.0", " 1", "١"])
    def test_an_id_that_is_not_a_decimal_integer_is_rejected(self, node_id):
        """GIVEN a node id the protocol cannot read as an integer
        WHEN the graph is built
        THEN it raises, naming the id.

        A word id runs here and nowhere else: the platform's exporter ``parseInt``s every endpoint
        and writes ``null`` for what it cannot read, so such a graph comes back with its wiring
        gone. ``"01"`` is refused for the opposite reason — it *is* readable, as ``1``, which makes
        it a second name for another node."""
        with pytest.raises(ValueError, match=r"Node id .* is not a decimal integer"):
            build({node_id: {"type": "int", "value": 1}}, {})

    def test_an_edge_key_that_is_not_a_decimal_integer_is_rejected(self):
        """GIVEN an edge keyed by a word
        WHEN the graph is built
        THEN it raises: the editor keys edges by integer, like nodes."""
        nodes = {"0": {"type": "float", "value": 1.0}, "1": {"type": "sqrt"}}

        with pytest.raises(ValueError, match=r"Edge key 'first' is not a decimal integer"):
            build(nodes, {"first": edge("0", "1", 0)})

    def test_an_endpoint_that_is_not_a_decimal_integer_is_rejected(self):
        """GIVEN an edge whose target is a word
        WHEN the graph is built
        THEN it raises before check 1 can call it an undeclared node — the id is unreadable, which
        is a different fault from naming a node that is not there."""
        nodes = {"0": {"type": "float", "value": 1.0}, "1": {"type": "sqrt"}}

        with pytest.raises(ValueError, match=r"Edge '0' target 'ghost' is not a decimal integer"):
            build(nodes, {"0": edge("0", "ghost", 0)})
...
class TestQualifiedIds:
    ...
    def test_the_mapping_is_exposed_on_the_graph(self):
        ...
        graph = Graph(
            {
                "0": {"type": "float", "value": 1.0, "qualified_id": "0"},
                "1": {"type": "sqrt", "qualified_id": "4_12"},
            },
            {"0": edge("0", "1", 0)},
            PORT_TABLE,
        )

        assert graph.qualified_ids == {"0": "0", "1": "4_12"}
    ...
    def test_a_node_declaring_none_is_rejected(self):
        ...
        with pytest.raises(ValueError, match=r"Node '0' declares no qualified_id"):
            Graph({"0": {"type": "float", "value": 1.0}}, {}, PORT_TABLE)
    ...
    def test_two_nodes_sharing_one_are_rejected(self):
        ...
        nodes = {
            "0": {"type": "float", "value": 1.0, "qualified_id": "same"},
            "1": {"type": "float", "value": 2.0, "qualified_id": "same"},
        }

        with pytest.raises(ValueError, match=r"Node '1' repeats qualified_id 'same'"):
    ...
    def test_one_that_cannot_be_a_filename_is_rejected(self):
        ...
        with pytest.raises(ValueError, match=r"Node '0' declares qualified_id 'a\.b'"):
            Graph({"0": {"type": "float", "value": 1.0, "qualified_id": "a.b"}}, {}, PORT_TABLE)
...
class TestSubgraphNodes:
    ...
    def test_a_node_typed_network_is_rejected_even_without_a_value(self):
        ...
        with pytest.raises(ValueError, match=r"Node '0' carries a nested workflow"):
            build({"0": {"type": "coral::Network", "node_type": "network"}}, {})
    ...
    def test_a_primitive_whose_value_is_a_dict_is_not_a_subgraph(self):
        ...
        graph = build({"0": {"type": "any", "value": {"nodes": 3}}}, {})
...
class TestInputPorts:
    ...
    def test_two_edges_on_one_port_are_rejected(self):
        ...
        edges = {"0": edge("0", "2", 0), "1": edge("1", "2", 0)}

        with pytest.raises(ValueError, match=r"Node '2' of type 'add' has 2 incoming edges"):
            build(nodes, edges)
    ...
    def test_port_index_out_of_range_is_rejected(self):
        ...
        nodes = {"0": {"type": "float", "value": 1.0}, "1": {"type": "add"}}

        with pytest.raises(ValueError, match=r"input ports \[3\]"):
            build(nodes, {"0": edge("0", "1", 3)})
    ...
    def test_contiguous_ports_are_accepted(self):
        ...
        edges = {"0": edge("0", "2", 0), "1": edge("1", "2", 1)}
...
class TestArity:
    ...
    def test_missing_connection_is_rejected(self):
        ...
        with pytest.raises(
            ValueError, match=r"Node '1' of type 'add' expects 2 inputs but received 1"
        ):
    ...
    def test_a_defaulted_parameter_still_has_to_be_wired(self):
        ...
        with pytest.raises(
            ValueError, match=r"Node '0' of type 'Widget' expects 1 inputs but received 0"
        ):
            build({"0": {"type": "Widget"}}, {})
    ...
    def test_method_instance_port_counts_as_an_input(self):
        ...
        nodes = {
            "0": {"type": "float", "value": 2.0},  # the Widget's size
            "1": {"type": "Widget"},
            "2": {"type": "float", "value": 3.0},  # resize's own parameter
    ...
    def test_a_zero_input_node_accepts_no_edges(self):
        ...
        wrong reason.
        """
        nodes = {"0": {"type": "float", "value": 1.0}, "1": {"type": "list_new"}}

        with pytest.raises(
            ValueError, match=r"Node '1' of type 'list_new' expects 0 inputs but received 1"
        ):
            build(nodes, {"0": edge("0", "1", 0)})
    ...
    def test_a_zero_input_node_alone_is_fine(self):
        ...
        assert build({"0": {"type": "list_new"}}, {}).order == ["0"]
...
class TestOutputPorts:
    ...
    def test_minus_one_is_rejected_on_a_multi_output_type(self):
        ...
        edges = {"0": edge("0", "1", 0), "1": edge("1", "2", 0, source_output=-1)}

        with pytest.raises(ValueError, match=r"Edge '1' reads output -1 .* valid: 0 to 2"):
            build(nodes, edges)
    ...
    def test_output_index_beyond_the_outputs_is_rejected(self):
        ...
        nodes = {"0": {"type": "float", "value": 1.0}, "1": {"type": "sqrt"}}
        edges = {"0": edge("0", "1", 0, source_output=7)}

        with pytest.raises(ValueError, match=r"Edge '0' reads output 7 .* valid: 0 or -1"):
            build(nodes, edges)
    ...
    def test_reading_from_a_node_that_returns_nothing_is_rejected(self):
        ...
        nodes = {"0": {"type": "float", "value": 1.0}, "1": {"type": "show"}, "2": {"type": "sqrt"}}
        edges = {"0": edge("0", "1", 0), "1": edge("1", "2", 0)}

        with pytest.raises(ValueError, match=r"Edge '1' .* 'show' returns nothing"):
            build(nodes, edges)
    ...
    def test_omitted_source_output_is_accepted_on_a_single_output_type(self):
        ...
        nodes = {"0": {"type": "float", "value": 1.0}, "1": {"type": "sqrt"}}
        edges = {"0": {"source": "0", "target": "1", "target_input": 0}}

        assert build(nodes, edges).order == ["0", "1"]
    ...
    def test_omitted_source_output_is_rejected_on_a_multi_output_type(self):
        ...
        edges = {"0": edge("0", "1", 0), "1": {"source": "1", "target": "2", "target_input": 0}}

        with pytest.raises(ValueError, match=r"Edge '1' reads output None"):
            build(nodes, edges)
    ...
    def test_each_output_of_a_tuple_can_be_read(self):
        ...
        nodes = {
            "0": {"type": "float", "value": 1.0},
            "1": {"type": "split"},
            "2": {"type": "sqrt"},  # reads output 0, a float
            "3": {"type": "label"},  # reads output 1, a str
        }
        edges = {
            "0": edge("0", "1", 0),
            "1": edge("1", "2", 0, source_output=0),
            "2": edge("1", "3", 0, source_output=1),
        }

        assert sorted(build(nodes, edges).order) == ["0", "1", "2", "3"]
...
class TestEdgeTypes:
    ...
    def test_identical_types_are_accepted(self):
        ...
        WHEN the graph is built
        THEN it is accepted."""
        assert build(*self._wire("float", "sqrt")).order == ["0", "1"]
    ...
    def test_int_into_float_is_accepted(self):
        ...
        assert build(*self._wire("int", "sqrt")).order == ["0", "1"]
    ...
    def test_float_into_int_is_rejected(self):
        ...
        with pytest.raises(ValueError, match=r"feeds float .* expects int"):
            build(nodes, {"0": edge("0", "1", 0)}, table)
    ...
    def test_bool_into_bool_is_accepted(self):
        ...
        assert build(nodes, {"0": edge("0", "1", 0)}, table).order == ["0", "1"]
    ...
    def test_any_on_the_source_skips_the_check(self):
        ...
        assert build(*self._wire("any", "sqrt")).order == ["0", "1"]
    ...
    def test_any_on_the_target_skips_the_check(self):
        ...
        assert build(*self._wire("str", "anything")).order == ["0", "1"]
    ...
    def test_a_collection_into_its_own_type_is_accepted(self, creator, consumer):
        ...
        WHEN the graph is built
        THEN it is accepted."""
        assert build(*self._wire(creator, consumer)).order == ["0", "1"]
    ...
    def test_a_collection_into_any_skips_the_check(self):
        ...
        assert build(*self._wire("list_new", "anything")).order == ["0", "1"]
    ...
    def test_any_into_a_collection_skips_the_check(self):
        ...
        assert build(*self._wire("any", "list_size")).order == ["0", "1"]
...
class TestLookups:
    ...
    def test_inputs_of_returns_edges_sorted_by_target_input(self):
        ...
        edges = {"0": edge("0", "2", 1), "1": edge("1", "2", 0)}

        incoming = build(nodes, edges).inputs_of("2")

        assert [e.target_input for e in incoming] == [0, 1]
        assert [e.source for e in incoming] == ["1", "0"]

    def test_inputs_of_a_source_node_is_empty(self):
        """GIVEN a primitive node
        WHEN its inputs are requested
        THEN the list is empty."""
        assert build({"0": {"type": "int", "value": 1}}, {}).inputs_of("0") == []
    ...
    def test_node_returns_the_definition(self):
        ...
        graph = build({"0": {"type": "int", "value": 7}}, {})

        assert graph.node("0") == {"qualified_id": "0", "type": "int", "value": 7}
    ...
    def test_ports_of_returns_the_table_entry_for_the_node_type(self):
        ...
        assert graph.ports_of("2") is PORT_TABLE["add"]
        assert graph.ports_of("0").kind == PRIMITIVE
    ...
    def test_edges_accept_a_plain_sequence(self):
        ...
        nodes = {"0": {"type": "float", "value": 16.0}, "1": {"type": "sqrt"}}

        graph = build(nodes, [edge("0", "1", 0)])
