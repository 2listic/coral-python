...
class PhiFlowCuboid:
    ...
    def __init__(self, center_x: float, center_y: float, half_size_x: float, half_size_y: float):
        self.cuboid = Cuboid(
            vec(x=center_x, y=center_y), half_size=vec(x=half_size_x, y=half_size_y)
        )
        print(
            f"PhiFlowCuboid created: center=({center_x}, {center_y}), half_size=({half_size_x}, {half_size_y})"
        )
...
def phiflow_iterate(
    ...
    v0 = (
        velocity_grid.get_grid()
        if isinstance(velocity_grid, PhiFlowStaggeredGrid)
        else velocity_grid
    )
    ...
    v_trj, s_trj, p_trj = iterate(
        step, batch(time=time_steps), v0, smoke0, None, dt=dt, substeps=substeps
    )
    print(
        f"phiflow_iterate executed: time_steps={time_steps}, dt={dt}, substeps={substeps}, obstacles={'Yes' if obstacles is not None else 'No'}"
    )
    return v_trj, s_trj, p_trj


def phiflow_plot_and_save(
    smoke_trajectory: Any,
    output_filename: str,
    frame_time: int,
    fps: int,
    dpi: int,
