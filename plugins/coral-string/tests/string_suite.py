...
#: worth having — `string` is what the DealiiX platform's `-p` contract names.
PLUGIN_NAME = "string"
...
MODULE_NAME = "coral_plugin_string"
...
#: The graphs this plugin owns. It ships none today: no editor export uses the StringProcessor. The
#: constant exists so adding one is a matter of dropping the file in — `test_graphs_validate.py` in
#: the other plugins discovers from disk, and this package can gain the same file unchanged.
