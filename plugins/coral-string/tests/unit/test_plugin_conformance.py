...
from coral_plugin_string import StringPlugin
...
class TestConformance:
    ...
    def test_it_subclasses_the_contract(self):
        ...
        assert issubclass(StringPlugin, Plugin)
    ...
    def test_it_can_be_instantiated(self):
        ...
        assert isinstance(StringPlugin(), Plugin)
    ...
    def test_the_surfaces_are_stable_across_calls(self, plugin):
        ...
        assert plugin.get_functions() == StringPlugin().get_functions()
        assert plugin.get_classes() == StringPlugin().get_classes()
...
class TestDeclaredSurface:
    ...
    def test_it_declares_the_expected_node_types(self, plugin):
        ...
        assert set(plugin.get_functions()) == {"print_text"}
        assert set(plugin.get_classes()) == {"StringProcessor"}
    ...
    def test_the_annotations_are_what_reject_a_swap(self, plugin):
        """GIVEN `StringProcessor.repeat`, whose two parameters have different types
        WHEN its annotations are read
        THEN they are `str` then `int`, in that order.

        The counterpart to `test_swapping_the_arguments_is_not_caught_at_run_time`: at run time
        `int * str` succeeds, so these two annotations are the *only* thing that makes a swapped wiring
        fail — and they make it fail before the graph runs. Written as an assertion because weakening
        either to `Any` would silently remove that protection.
        """
        signature = inspect.signature(plugin.get_classes()["StringProcessor"].repeat)
        parameters = [p for name, p in signature.parameters.items() if name != "self"]

        assert [p.name for p in parameters] == ["text", "times"]
        assert [p.annotation for p in parameters] == [str, int]
