...
plugin, so it survives to report a visible skip instead of the whole directory quietly vanishing
(see ``conftest.py``).
"""
...
import pytest
from pypde_suite import MODULE_NAME, PLUGIN_GROUP, PLUGIN_NAME, requires_this_plugin
...
def test_the_entry_point_name_is_the_one_the_platform_uses():
    ...
    The name is the whole of the plugin's identity: it is what ``-p pypde`` selects, what the DealiiX
    platform's ``coralPluginPath`` lists, and what ``discover()`` returns. Renaming it is a breaking
    change for the platform, so it is asserted against a hand-written constant.
    """
...
def test_the_entry_point_resolves_to_this_plugin_class():
    ...
    assert found.load() is PyPDEPlugin
...
def test_this_distribution_is_registered_under_its_declared_name():
    ...
    round fails loudly instead. It reads metadata only, so it never imports the plugin.
    """
    mine = {
