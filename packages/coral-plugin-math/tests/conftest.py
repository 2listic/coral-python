...
The directory survives ``uv pip uninstall coral-plugin-math``, so it must notice: when the entry point
...
The shared constants live in ``math_suite.py`` rather than here; see that module's docstring for why.
"""
...
from math_suite import INSTALLED  # noqa: E402  (needs the sys.path line above)
