...
Written to match the reference backend, which is the
...
It diverges from the reference backend on one point, deliberately: it invents a
``<node_id>_auto_<counter>`` name for
...
def qualified_ids(nodes: Mapping[str, dict]) -> Dict[str, str]:
    ...
    - **empty.** It would write ``.running``, a dotfile with no node name in it. (The reference backend falls back
      to the node id here; we do not, for the same reason we reject a missing field.)
...
class NodeStatusDir:
    ...
    def node(self, qualified_id: str) -> Generator[None, None, None]:
        ...
        As in the reference backend, a failed node keeps its ``.running`` file: the pair is what tells the platform
