...
# A collection could in principle be one too — the C++ backend registers `std::set<unsigned int>` as
# an elementary type and carries the literal as the string "[1, 2, 3, 4, 5]" — but coral-python does
# not offer it (see issue #35). An *empty* collection primitive would duplicate `list_new()` /
# `set_new()` / `dict_new()` in `builtin_nodes.py`, two node types for one behaviour; a *literal*
