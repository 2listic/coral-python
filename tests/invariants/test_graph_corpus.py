...
build graphs in memory (``coral-app/tests/test_graph.py`` uses ``"a"``, ``"b"``, ``"c"`` throughout),
while anything the repo hands to the platform has to be something the platform can read back.

It lives here, in the repo-level invariants, rather than in each package's suite: the rule is about
the graph *format* the platform exports, identical whether a graph names ``math``, ``phiflow`` or no
plugin at all, and no single package can assert it of the others. Graph JSON is data, so this needs
nothing installed.

Files are discovered from disk — under every package's ``examples/`` and ``tests/`` — so a new graph
is covered the day it is added. A JSON file with no ``workflow.nodes`` is not a graph and is skipped
(that is how the registry goldens fall out); ``test_some_graphs_were_found`` fails loud if that skip
ever swallows everything, because a discovery that finds nothing would otherwise pass as green.
"""
...
#: The distributions, in the order the workspace lists them.
PACKAGES = [
    REPO_ROOT / "coral-core",
    REPO_ROOT / "coral-app",
    *sorted((REPO_ROOT / "plugins").glob("coral-*")),
]

#: Where a package may keep graphs: the examples it ships, and its own test data.
SEARCH_DIRS = tuple(
    directory
    for package in PACKAGES
    for directory in (package / "examples", package / "tests")
    if directory.is_dir()
)
...
def test_some_graphs_were_found():
    """GIVEN every package's examples and test data
