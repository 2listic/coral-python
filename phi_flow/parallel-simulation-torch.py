...
def step(v, s, p, dt=1.):
    ...
    v, p = fluid.make_incompressible(v, obstacle, Solve('scipy-CG', 1e-3, x0=p))
    return v, s, p
...
print(f'{v0=}')
print(f'{smoke0=}')
v_trj, s_trj, p_trj = iterate(step, batch(time=30), v0, smoke0, None, dt=.5, substeps=3)
# %%
# plot(obstacle, inflow, s_trj, animate='time', overlay='args')
# anim.save('obstacle.gif')
# anim.save('simulation.mp4', writer='ffmpeg', fps=30, dpi=150)
