...
- **Graph** (consumed by `run`) — `{"workflow": {"nodes": {...}, "edges": {...}}, ...}`, where each
  node is *lean*: just `{"qualified_id": "...", "type": "...", "value": ...}` (primitives) or
  `{"qualified_id": "...", "type": "..."}` (everything else). No `node_type`, no `method_name` —
  what a node **is** follows purely from its `type` string: `nodeports.py` decides the kind when it builds the port table, and the executor reads
...
- **Per-node execution status (done).** `--touch-dir` now emits one empty
  ...
  defaults to the cwd, every node is a task (primitives included), and a failed touch mid-run is not
  fatal. On the fourth — C++ treats `qualified_id` as optional and invents a
  `<node_id>_auto_<counter>` name for a node that omits it — we deliberately differ and require the
  field. Details and the two deliberate divergences — that one, and not wrapping the failing node's
  exception — are in [Per-node execution status](../CLAUDE.md#per-node-execution-status).
